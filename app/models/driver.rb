class Driver < ActiveRecord::Base
  def self.pick
    User.all.each do |u|
      # Use PhantomJS to handle crawling
      ...
      retries = 0
      begin
        username = fields.find { |f| f.attribute('placeholder')['Username'] }.send_keys u.username
        password = fields.find { |f| f.attribute('placeholder')['Password'] }.send_keys u.password
      rescue
        if retries < 20
          retries += 1
          retry
